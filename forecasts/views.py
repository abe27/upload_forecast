...
def export_excel(request, id):
    ...
    rows = ForecastErrorLogs.objects.filter(file_name=id, is_success=False).values_list(
        "part_code",
        "part_no",
        "part_name",
        "supplier",
...
def export_forecast(request, id):
    ...
    book_ro = open_workbook(
        os.path.join(settings.BASE_DIR, "static/file_template", "templ_pds.xls"),
    ...
    sheet1.write(
        0,
        0,
        f"Forecast {fcHeader.forecast_plan_id} / {fcHeader.forecast_revise_id}",
        style,
    )
