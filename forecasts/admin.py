...
class ForecastYearFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        """
        dte = datetime.now()
        data = OnYearList.objects.filter(value__gte=int(dte.year) - 1).values()[:3]
        docs = []
        rnd = 0
        for i in data:
            docs.append((i["id"], f"{i['name']}"))
...
class ForecastSupplierFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        if request.user.groups.filter(name="Supplier").exists():
            ...
            for u in usr:
                sup = Supplier.objects.filter(id=u.supplier_id).values(
                    "id", "code", "name"
                )
                for i in sup:
                    docs.append((i["id"], f"{i['code']}-{i['name']}"))
...
class ForecastAdmin(admin.ModelAdmin):
    ...
    def total_qty(self, obj):
        if obj.forecast_total_qty > 0:
            return format_html(
                f"<span class='text-xs badge text-bg-info'>{int(obj.forecast_total_qty):,}</span>"
            )

        return format_html(
            f"<span class='text-xs badge text-bg-danger'>{obj.forecast_total_qty}</span>"
        )
    ...
    def download_forecast(self, obj):
        ...
        return format_html(
            f"<a class='btn btn-sm {txtClass}' href='/forecast/export_forecast/{obj.id}' target='_blank'>Download</a>"
        )
    ...
    def get_model(self, obj):
        return format_html(
            f"<span class='text-xs badge badge-success'>{obj.part_model_id}</span>"
        )
    ...
    def get_qty(self, obj):
        if obj.forecast_qty > 0:
            return format_html(
                f"<span class='text-xs badge text-bg-secondary'>{int(obj.forecast_qty):,}</span>"
            )

        return format_html(
            f"<span class='text-xs badge text-bg-danger'>{obj.forecast_qty}</span>"
        )
    ...
    def get_item(self, obj):
        if obj.forecast_item > 0:
            return format_html(
                f"<span class='text-success text-bold'>{obj.forecast_item}</span>"
            )

        return format_html(
            f"<span class='text-danger text-bold'>{obj.forecast_item}</span>"
        )
    ...
    def status(self, obj):
        try:
            ...
            return format_html(
                f"<span class='text-xs badge {txtClass}'>{data[1]}</span>"
            )
    ...
    def get_queryset(self, request):
        ...
        if request.user.groups.filter(name="Supplier").exists():
            ...
            obj = qs.filter(
                forecast_status="1", supplier_id__in=sup_id, forecast_qty__gt=0
            )
            return obj
